...
(function (V) {
    var E = V.fn.domManip, S = "_tmplitem", F = /^[^<]*(<[\w\W]+>)[^>]*$|\{\{\! /, U = {}, Q = {}, R,
        G = {key: 0, data: {}}, N = 0, T = 0, K = [];
...
(function (b) {
    ...
    b.closeLightBox = function () {
        var e = b("#lightBox"), c = b("#overlay"), d = b("#lbContent");
        if (e.data("opts")) {
            url = e.data("opts").url;
            if (url.indexOf("#") == 0) {
    ...
    b.lightBox = function (l) {
        var e = {
        ...
        }, c = b.extend(e, l), g = b(c.overlay), f = b(c.lightBox), h = b(c.content), d = function () {
            if (c.url.indexOf("#") == 0) {
            ...
            h.css({height: ""});
            f.css({top: "50%", left: "50%"});
            if (c.size) {
                f.addClass(c.size)
            }
            var p = f.height(), n = b(window).height(), m = (p < n ? p : n) - 50;
            if (p > n) {
                h.css({height: m - 50});
                h.find(".close-holder").css({right: 10})
            }
            f.css({marginTop: -m / 2});
            if (f.draggable) {
                f.draggable({handle: ("div.close-holder")})
            }
        };
        f.data("opts", c);
        g.removeClass("hide");
    ...
    b.confirm = function (e) {
        var d = {
            title: "确认", msg: "是否确认删除？", btntext: "删除", showyes: false, confirm: function () {
            }, cancel: function () {
            }
        };
        var c = b("#overlay"), f = b("#deleteBox");
        if (e) {
            b.extend(d, e)
        }
        d.confirm = function () {
        ...
        c.removeClass("hide");
        f.removeClass("hide");
        f.find(".btnconfirm").unbind().bind({click: d.confirm});
        f.find(".btncancel").unbind().bind({click: d.cancel});
        if (f.draggable) {
            f.draggable({handle: f.find("h4.title")})
        }
    };
    b.alert = function (e, f) {
        var d = b("#overlay"), c = b("#alertBox");
        if (d.length == 0) {
        ...
        if (c.draggable) {
            c.draggable({handle: c.find("h4.title")})
        }
    };
    b.malert = function (c, g) {
        var e = b("#divErrorParent"), d = b("#divErrMsgMobile");
        if (e.length == 0) {
        ...
        d.text(c).animate({opacity: "1"}, "500");
        setTimeout(f, 2000);

        function f() {
            b("#divErrMsgMobile").animate({opacity: "0"}, "500", function () {
                b("#parentDiv").remove();
                if (g) {
                    g()
                }
    ...
    b.fn.setErrorFlag = function (c) {
        if (!this.hasClass("error")) {
            ...
            if (this.effect) {
                this.effect("shake", {distance: 10, times: 3}, 20)
            }
        }
    };
    ...
    b.initDate = function (c, d) {
        ...
        if (b.fn.datepicker) {
            c.find("input.datepincker").datepicker({
                ...
                monthNames: Date.monthNames || ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"],
                monthNamesShort: Date.abbrMonthNames || ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二"],
                dayNames: Date.abbrDayNames || ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"],
                dayNamesMin: Date.dayNames || ["日", "一", "二", "三", "四", "五", "六"]
            }).bind({
                change: function () {
