  ...
  class OssGreenService extends app.Service {
    ...
    * checkImage(url) {
      ...
      if (data.Code === 'Success' && data.ImageResults.ImageResult.length > 0) {
        const imageResult = data.ImageResults.ImageResult[0];
        const imageUrl = imageResult.ImageName;
        const taskId = imageResult.TaskId;

        // 获取结果
        const pornResult = imageResult.PornResult;
        const ocrResult = imageResult.OcrResult;
        const illegalResult = imageResult.IllegalResult;
        const sensitiveFaceResult = imageResult.SensitiveFaceResult;
        const adResult = imageResult.AdResult;
        const qrcodeResult = imageResult.QrcodeResult;
